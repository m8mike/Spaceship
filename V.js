strokeJoin(ROUND);
textFont(createFont("serif", 10), 12);
var space = false;
var gravity = 0.1;
var thrust = 0.25;
var spaceship = null;
...
Spaceship.prototype.update = function() {
    if (space) {
        if (!this.pressed && gui.fuel > 0) {
            this.pressed = true;
            this.direction = -this.direction;
        }
        if (gui.fuel > 0) {
            gui.fuel-=0.6;
            if (this.ySpeed*this.direction < 0) {
                this.ySpeed += gravity*this.direction;
            }
            //this.ySpeed -= thrust*50;
            if (this.smokeDelay <= 0) {
                this.smokeDelay = 3;
                this.smoke.push(new Smoke(this.x-50, this.y+random(-10, 10)));
            }
        }
    } else {
        this.ySpeed += gravity*this.direction;
