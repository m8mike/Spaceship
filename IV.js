...
GUI.prototype.update = function() {
    ...
    pushMatrix();
    fill(255, 255, 255);
    text("score: " + this.distance1, 10*this.scaleRatio, 10);
    translate(0.7,0.7);
    colorMode(HSB);
    fill(this.colorOffset,255,255);
    this.colorOffset = ++this.colorOffset%255;
    colorMode(RGB);
    text("score: " + this.distance1, 10*this.scaleRatio, 10);
    popMatrix();
    pushMatrix();
    fill(255, 255, 255);
    text("best: " + this.best, 220 + 10*this.scaleRatio, 10);
    translate(0.7,0.7);
    colorMode(HSB);
    fill(this.colorOffset,255,255);
    this.colorOffset = ++this.colorOffset%255;
...
Asteroid.prototype.update = function() {
    ...
    if (this.colorOffset > 250) {
        this.colorDelta = -2;
    } else if (this.colorOffset < 1) {
        this.colorDelta = 2;
    }
    this.colorOffset += this.colorDelta;
    this.draw();
};
Asteroid.prototype.draw = function() {
    ...
    endShape();
    popMatrix();
    fill(51, 255, this.colorOffset, 200);
    stroke(255, 255, 255);
    beginShape();
    for (var i = 0; i < 7; i++) {
        vertex(this.size * Math.cos(i * 60 / 180 * Math.PI), 
...
SpeedUp.prototype.draw = function() {
    ...
    pushMatrix();
    translate(this.x, this.y);
    stroke(255, 255 - this.colorOffset, 0);
    fill(255, 170 - this.colorOffset, 0);
